...
// birbirine bağımlıydı (bkz. CLAUDE.md'deki `migrateBrandsTableIfNeeded` guard
// zinciri tuzağı). Artık:
//
//   - Sıra `MIGRATIONS` dizisinde AÇIK ve tek yerde.
//   - Hangisinin çalıştığı `schema_migrations` tablosunda kayıtlı; ikinci
//     açılışta hiçbiri tekrar çalışmıyor.
//   - Yeni bir göç eklemek = diziye tek satır. "Sütun var mı" guard'ı yazmak
//     artık ZORUNLU değil (eskiler kendi guard'larını koruyor, ikinci güvenlik
//     ağı olarak duruyorlar).
//
// Mevcut bir veritabanında `schema_migrations` yoksa hepsi bir kez daha çalışır
// ve kaydedilir — tek seferlik geri doldurma. Hepsi idempotent olduğu için bu
// güvenli; zaten tasarımlarının tamamı buna dayanıyordu.
...
function migrateCalendarEventColorIfNeeded(db: DatabaseSync): void {
  ...
  if (!columns.some((column) => column.name === "color_key")) {
    db.exec(`ALTER TABLE calendar_events ADD COLUMN color_key TEXT NOT NULL DEFAULT 'auto'
      CHECK (color_key IN ('auto','lavender','sage','purple','coral','amber','orange','cyan','slate','blue','green','rose'))`);
  }
}

// Google Takvim'in 11 etkinlik renginin tamamı destekleniyor. Eski v03
// veritabanlarında color_key CHECK'i sekiz anahtarla sınırlıydı; SQLite CHECK'i
// yerinde değiştiremediği için güncel tablonun bütün sütunlarını veri kaybetmeden
// yeniden kuruyoruz. Eski tabloyu yeniden adlandırmıyoruz; böylece rapor ve
// bildirim tablolarının calendar_events FK hedefi değişmeden kalır.
function migrateCalendarEventPaletteIfNeeded(db: DatabaseSync): void {
  const row = db
    .prepare(`SELECT sql FROM sqlite_master WHERE type='table' AND name='calendar_events'`)
    .get() as { sql: string } | undefined;
  if (!row || row.sql.includes("'lavender'")) return;

  db.exec("PRAGMA foreign_keys = OFF");
  try {
    db.exec("BEGIN");
    db.exec(`
      CREATE TABLE calendar_events_new_palette_migration (
        id                TEXT PRIMARY KEY,
    ...
    `);
    db.exec(`
      INSERT INTO calendar_events_new_palette_migration
        (id, brand_id, type, color_key, title, description, start_at, end_at,
         all_day, location, guest_visible, google_event_id, google_etag,
         google_updated_at, sync_status, sync_error, deleted_at,
         created_by_account_id, created_at, updated_at, last_synced_at)
      SELECT
         id, brand_id, type, color_key, title, description, start_at, end_at,
         all_day, location, guest_visible, google_event_id, google_etag,
         google_updated_at, sync_status, sync_error, deleted_at,
    ...
    db.exec(`ALTER TABLE calendar_events_new_palette_migration RENAME TO calendar_events`);
    db.exec(`CREATE INDEX IF NOT EXISTS idx_calendar_events_range ON calendar_events(start_at, end_at)`);
    db.exec(`CREATE INDEX IF NOT EXISTS idx_calendar_events_brand ON calendar_events(brand_id, start_at)`);
    db.exec(`CREATE INDEX IF NOT EXISTS idx_calendar_events_sync ON calendar_events(sync_status, updated_at)`);
    if (db.prepare("PRAGMA foreign_key_check").all().length > 0) {
      throw new Error("Takvim renk paleti göçü foreign key bütünlüğünü bozdu.");
    }
    db.exec("COMMIT");
  } catch (error) {
    db.exec("ROLLBACK");
...
// Marka rengi isimden türetilmez: yeniden adlandırma veya sıra değişikliği
// görsel kimliği değiştirmesin diye bir kez atanıp brands satırında saklanır.
function migrateBrandAccentHueIfNeeded(db: DatabaseSync): void {
  const exists = db
    .prepare(`SELECT 1 FROM sqlite_master WHERE type='table' AND name='brands'`)
    .get();
  if (!exists) return;
  const columns = db.prepare(`PRAGMA table_info(brands)`).all() as { name: string }[];
  if (!columns.some((column) => column.name === "accent_hue")) {
    db.exec(`ALTER TABLE brands ADD COLUMN accent_hue INTEGER NOT NULL DEFAULT 210 CHECK (accent_hue BETWEEN 15 AND 329)`);
  }
  const rows = db
    .prepare(`SELECT id FROM brands ORDER BY sort_order, name COLLATE NOCASE, id`)
    .all() as { id: string }[];
  const update = db.prepare(`UPDATE brands SET accent_hue = ? WHERE id = ?`);
  rows.forEach((row, index) => update.run(brandAccentHueForIndex(index), row.id));
}
...
export function seedTaskTemplatesIfNeeded(db: DatabaseSync): void {
...
// DİKKAT: bu bir GÖÇ DEĞİL, her açılışta çalışması gereken bir UZLAŞTIRMA adımı.
// Kategoriler kullanıcı tarafından yönetiliyor ve `brands.cluster` ile
// `clusters.id` arasında bilinçli olarak FK yok; biri markaya elle yeni bir
// kategori id'si yazarsa o kategori `clusters` tablosunda sahipsiz kalır. Bu
// fonksiyon sahipsizleri her bağlantı açılışında adopte eder. Tek sefer
// çalıştırılırsa (yalnızca `MIGRATIONS` içinden) sonradan eklenen kategori
// hiç görünmez — bir test tam olarak bunu yakalıyor.
// `MIGRATIONS` listesinde de duruyor: eski bir DB'de SIRA önemli (001'den sonra,
// 003'ten önce). İki kez çalışması zararsız, `INSERT OR IGNORE`.
export function seedClustersIfNeeded(db: DatabaseSync): void {
  db.exec(`
...
const MIGRATIONS: readonly { id: string; run: (db: DatabaseSync) => void }[] = [
  { id: "001-brands-table", run: migrateBrandsTableIfNeeded },
  { id: "002-seed-clusters", run: seedClustersIfNeeded },
  { id: "003-brands-drop-cluster-check", run: migrateBrandsDropClusterCheckIfNeeded },
  { id: "004-content-items-table", run: migrateContentItemsTableIfNeeded },
  { id: "005-content-items-archived", run: migrateContentItemsArchivedIfNeeded },
  { id: "006-content-item-types", run: migrateContentItemTypesIfNeeded },
  { id: "007-tasks-table", run: migrateTasksTableIfNeeded },
  { id: "008-tasks-repeat", run: migrateTasksRepeatIfNeeded },
  { id: "009-tasks-reporting", run: migrateTasksReportingIfNeeded },
  { id: "010-tasks-archived-at", run: migrateTasksArchivedAtIfNeeded },
  { id: "011-people-profiles", run: migratePeopleProfilesIfNeeded },
  { id: "012-people-department", run: migratePeopleDepartmentIfNeeded },
  { id: "013-people-auth", run: migratePeopleAuthIfNeeded },
  { id: "014-people-username", run: migratePeopleUsernameIfNeeded },
  { id: "015-v03-task-columns", run: migrateV03TaskColumnsIfNeeded },
  { id: "016-task-template-difficulty", run: migrateTaskTemplateDifficultyIfNeeded },
  { id: "017-v03-accounts", run: migrateV03AccountsIfNeeded },
  { id: "018-v03-notification-columns", run: migrateV03NotificationColumnsIfNeeded },
  { id: "019-calendar-event-color", run: migrateCalendarEventColorIfNeeded },
  { id: "020-social-posts-unique", run: migrateSocialPostsUniqueIfNeeded },
  { id: "021-client-requests-archive", run: migrateClientRequestsArchiveIfNeeded },
  // SIRA ÖNEMLİ: 001 ve 003 brands tablosunu SABİT bir sütun listesiyle yeniden
  // kuruyor; aşağıdaki iki ALTER onlardan sonra çalışmalı, yoksa ekledikleri
  // sütun rebuild sırasında düşer.
  { id: "022-brands-stats-updated", run: migrateBrandsStatsUpdatedIfNeeded },
...
export function runPendingMigrations(db: DatabaseSync): void {
  db.exec(
  ...
  const record = db.prepare("INSERT OR IGNORE INTO schema_migrations (id) VALUES (?)");
