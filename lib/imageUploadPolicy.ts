...
export const IMAGE_UPLOAD_HINT = "En fazla 6 görsel · dosya başına 8 MB · toplam 20 MB · PNG, JPG, GIF, WEBP";
...
export function validateImageFiles(images: Pick<File, "name" | "size" | "type">[]): void {
  ...
  for (const image of images) {
    if (!image.size || image.size > MAX_IMAGE_SIZE) throw new Error(`${image.name}: dosya boş veya 8 MB sınırını aşıyor.`);
