  ...
  return (
    <div>
      ...
      <div className="grid items-start gap-5 lg:grid-cols-[22rem_minmax(0,1fr)]">
        <ActionForm
          action={createGuestTaskAction}
          redirectPathPrefix="/guest/tasks/"
          className="space-y-4 rounded-xl border border-border-default bg-surface p-4"
        >
          <h2 className="text-sm font-semibold text-foreground">Yeni görev aç</h2>
          ...
          <label className="grid gap-1.5 text-xs font-medium text-secondary">Görseller<input name="images" type="file" accept="image/png,image/jpeg,image/gif,image/webp" multiple className="text-xs" /></label>
          <SubmitButton pendingLabel="Gönderiliyor…" className="min-h-10 w-full rounded-lg bg-brand-600 px-3 text-sm font-semibold text-white hover:bg-brand-500 disabled:cursor-wait disabled:opacity-70">Görevi gönder</SubmitButton>
          <p className="text-[11px] leading-5 text-muted">İstenen tarih talebinizi belirtir. Ekibin iç teslim tarihi ayrıca planlanır ve paylaşılmaz.</p>
        </ActionForm>
