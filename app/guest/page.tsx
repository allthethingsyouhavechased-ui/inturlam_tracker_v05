...
export default async function GuestDashboardPage() {
  ...
  return (
    <div>
      <PageHeader eyebrow="MARKA PORTALI" title={actor.brand.name} description="Aylık ilerleme, açtığınız görevler ve paylaşılan etkinlikler." />
      <div className="grid gap-4 md:grid-cols-3">
        <section className="rounded-xl border border-border-default bg-surface p-5 md:col-span-2"><p className="text-xs font-semibold tracking-wide text-muted">BU AYKİ MARKA İLERLEMESİ</p><p className="mt-3 text-4xl font-semibold tracking-tight text-foreground">{progress.percent === null ? "Bu ay plan yok" : `%${progress.percent}`}</p>{progress.percent !== null && <><div className="mt-4 h-2 overflow-hidden rounded-full bg-surface-subtle"><div className="h-full rounded-full bg-brand-600" style={{ width: `${progress.percent}%` }} /></div><p className="mt-2 text-xs text-muted">{progress.task_count} planlanmış görev</p></>}</section>
        <section className="rounded-xl border border-border-default bg-surface p-5"><p className="text-xs font-semibold tracking-wide text-muted">GÖREVLER</p><p className="mt-3 text-3xl font-semibold text-foreground">{tasks.length}</p><Link href="/guest/tasks" className="mt-4 inline-flex text-sm font-semibold text-brand-600">Tümünü aç →</Link></section>
      </div>
      <section className="mt-5 grid grid-cols-2 gap-2 sm:grid-cols-5">{(["Beklemede", "DevamEdiyor", "Incelemede", "Onaylandi", "Yayinlandi"] as const).map((status) => <div key={status} className="rounded-xl border border-border-default bg-surface px-3 py-3"><p className="text-[11px] text-muted">{TASK_STATUS_LABEL[status]}</p><p className="mt-1 text-xl font-semibold text-foreground">{counts.get(status) ?? 0}</p></div>)}</section>
