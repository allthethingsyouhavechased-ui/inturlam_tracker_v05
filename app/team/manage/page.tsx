...
import RequestReviewerToggle from "@/components/RequestReviewerToggle";
...
import { listRequestReviewers } from "@/lib/repositories/requestReviewers";
...
export default async function TeamManagementPage() {
  ...
  // Talep değerlendirme yetkisi ayrıca verilmiş kişiler (yöneticiler tabloda
  // olmadan da değerlendirebiliyor, bkz. lib/requestAccess.ts).
  const reviewerIds = new Set(listRequestReviewers().map((row) => row.person_id));
        ...
        <div className="overflow-hidden rounded-xl border border-border-default bg-surface">
          {people.map((person, index) => (
            ...
            >
              ...
              <div className="grid grid-cols-2 items-center gap-2 sm:grid-cols-[9rem_8.5rem_9.5rem_9rem_10rem_4.5rem] sm:justify-items-start">
                <Link
                ...
                {/* Talep değerlendirme yetkisi: yöneticilerde zaten var, bu
                    düğme yalnızca yönetici olmayanlar için anlamlı. */}
                {canEditRoles && person.is_manager !== 1 ? (
                  <RequestReviewerToggle
                    personId={person.id}
                    isReviewer={reviewerIds.has(person.id)}
                  />
                ) : <span aria-hidden="true" />}
